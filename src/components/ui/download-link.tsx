...
export interface DownloadLinkProps extends Omit<React.ComponentProps<typeof Link>, 'to'> {
    href: string;
    asChild?: boolean;
}
...
const DownloadLink = React.forwardRef<HTMLAnchorElement, DownloadLinkProps>(
    ({ className, href, asChild = false, ...props }, ref) => {
        ...
        return (
            <Comp ref={ref} to={href} className={cn(className)} onClick={handleClick} {...props} />
        );
    }
);
