  ...
  ({ torrent, detailed = false, asChild = false, className, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        {detailed && torrent.bit_depth && <><span>•</span><span>{torrent.bit_depth}-bit</span></>}
        {detailed && torrent.audio_channels && <><span>•</span><span>{torrent.audio_channels}</span></>}
        {torrent.type && <><span>•</span><span>{torrent.type}</span></>}
      </Comp>
    );
  }
);
